...
class FooterLinkController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->has('search')) {
            $data['footer_link'] = FooterLink::with('category')->where('pickup', 'LIKE', '%' . $request->search . '%')->orderBy('id', 'DESC')->paginate(20);
        } else {
            $data['footer_link'] = FooterLink::with('category')->orderBy('id', 'DESC')->paginate(20);
        }
        return view('Admin.footer_link.index', compact('data'));
    }
    ...
    {
        $data['category'] = FooterLinkCategory::where('status', '1')->get();
    ...
    public function update(Request $request, string $id)
    {
        ...
        Seo::where('page_url', $slug)->update([
            'page_url' => $slug ?? '',
            'title' => $request->title ?? '',
            'keyword' =>  '',
            'description' =>  '',
